...
router.post('/:id/member', async (req, res) => {
  ...
  if (result === null) {
    ...
    let { privategroups } = user;
    privategroups = privategroups.concat([id]);
    members = members.concat([userID]);
    joinRequest = joinRequest.filter((user) => user !== userID);
...
router.delete('/:id/member', async (req, res) => {
  ...
  if (result === null) {
    ...
    let { privategroups } = user;
    privategroups = privategroups.filter((group) => group !== id);
...
router.delete('/:id/deletion', async (req, res) => {
  ...
  if (result === null) {
    ...
    let { deletionRequest } = result;
    deletionRequest = deletionRequest.filter((request) => (request.from !== userID) || (request.postID !== postID));
    await Group.updateOne(
      {
        id,
      },
