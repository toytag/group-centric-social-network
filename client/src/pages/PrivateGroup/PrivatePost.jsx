...
import Comment from '../../components/PrivateGroup/Comment';
...
export default function PrivatePost() {
  ...
  useEffect(async () => {
    ...
    if (data === null) {
      ...
      const group = await fetchGroup.fetchGroup(data.group);
      if (group === null) {
        navigate('/wrongpage');
      } else if (group.members.indexOf(userID) < 0) {
        navigate('/permission');
      } else {
          ...
          { post
            ? (
              <Card elevation={0}>
                <CardHeader
                  action={
                    <IconButton onClick={() => handleClickHome(post.group)}><Home /></IconButton>
                  }
                  title={post.title}
            ...
            : <div />}
          {post && utils.checkUserInPrivateGroup(userInfo, post.group)
            ? (
