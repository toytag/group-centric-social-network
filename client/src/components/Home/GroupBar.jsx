...
import {
  Paper, Chip, Typography, Collapse, IconButton, FormControl, Select, InputLabel, Divider,
} from '@mui/material';
...
export default function GroupBar({ groups, userInfo }) {
  ...
  const handleDelete = (chipToDelete) => () => {
    setChip((chips) => chips.filter((chipIter) => chipIter !== chipToDelete));
  };

  const handleClick = (chipToAdd) => () => {
    if (chip.indexOf(chipToAdd) < 0) {
      setChip(chip.concat(allChips.filter((chipIter) => chipIter === chipToAdd)));
    }
  };
