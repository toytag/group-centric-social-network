...
export default function PostCard({ post, userInfo, group }) {
  ...
  const handleClickHide = async () => {
    await fetchUser.hidePost(userInfo.id, post._id, post.group, 'public');
    navigate(0);
  };
  ...
  const canHide = userInfo
    ? (post.author !== userInfo.id) && utils.checkUserInPublicGroup(userInfo, group.id) : false;
      ...
      <Card style={cardStyle} elevation={3}>
        ...
        <CardHeader
          action={(
            <div>
              {canHide
                ? <IconButton onClick={handleClickHide}><VisibilityOff /></IconButton> : null}
