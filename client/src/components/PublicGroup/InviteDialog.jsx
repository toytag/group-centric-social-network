...
export default function InviteDialog({ group, userID }) {
  ...
  const handleInvite = async (userToInvited) => {
    await fetchUser.invite(userID, userToInvited, group.id, 'public');
    navigate(0);
  };
    ...
    <Paper className={classes.root}>
      ...
      {existed && (
        <div className={classes.searchInfo}>
          <Typography>The user is already a group member.</Typography>
        </div>
      )}
      {nobody && (
        <div className={classes.searchInfo}>
