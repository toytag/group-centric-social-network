...
enableFetchMocks();
const lib = require('../../utils/fetchPublicGroup');

describe('fetchPublicGroup', () => {
  test('createPublicGroup', async () => {
    const data = await lib.createPublicGroup('group', 'wrf', 'sport');
    expect(data).toBe(200);
  });

  test('fetchAllPublicGroups', async () => {
    fetch.mockResponse(JSON.stringify(
      [{ id: 'group' }],
    ));
    const data = await lib.fetchAllPublicGroups();
